...
BEARER_TOKEN = os.getenv("SL_INTUIT_TITLE_TASK_TOKEN", "1234")
timeout = int(os.getenv("SL_TASK_TIMEOUT", "1000"))
page_title = os.getenv("INTUIT_TITLE", "Intuit Snowflake Agent")
title = os.getenv("INTUIT_TITLE", "Intuit Snowflake Agent")
...
if "Inuit_messages" not in st.session_state:
    st.session_state.Intuit_messages = []

for message in st.session_state.Intuit_messages:
...
if prompt:
    ...
    st.session_state.Intuit_messages.append({"role": "user", "content": prompt})
    with st.spinner("Working..."):
        try:
            ...
            if response.status_code == 200:
                ...
                st.session_state.Intuit_messages.append({"role": "assistant", "content": f"```json\n{pretty_result}\n```"})
                st.rerun()
            else:
